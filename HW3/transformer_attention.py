...
class AttentionQKV(nn.Module):
    ...
    def forward(self, queries, keys, values, mask=None):
        ...
        #similarity = th.einsum('b h i k, b h j k -> b h i j', queries, keys)/th.sqrt(key_dim)# Compute the similarity according to the QKV formula
        similarity = (queries @ keys.transpose(-2, -1))/th.sqrt(key_dim)
        ...
        #output =  th.einsum('b h i j , b h j d -> b h i d', weights, values)# Obtain the output

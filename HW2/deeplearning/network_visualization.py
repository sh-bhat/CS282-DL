...
def compute_saliency_maps(X, y, model):
    ...
    grads = torch.autograd.grad(scores, X)  #does not pass autograder
    saliency, _ = torch.max(grads[0].abs(), dim=1)
    #scores.backward()
...
def make_fooling_image(X, target_y, model):
    ...
    while True:
        ...
        if prediction == target_y:
            ...
            grads = torch.autograd.grad(scores[:, target_y], X_fooling)[0]
            dX = learning_rate * grads / torch.norm(grads)
            X_fooling.data += dX
    ##############################################################################
    #                             END OF YOUR CODE                               #
    ##############################################################################
...
def update_class_visulization(model, target_y, l2_reg, learning_rate, img):
    ...
    I_out = score_y - l2_reg*(torch.norm(img)**2)
    I_out.backward()
